...
async function describeCols(table: string): Promise<Set<string>> {
  const sql = `DESCRIBE TABLE ${fqtn(table)}`;
  const rows: any[] = await runSql(sql);
  ...
  try {
    ...
    if (programIdInAttendance) {
      ...
      return {
        ...
        jsonBody: {
          error:
            "Could not find a way to link attendance to programs. " +
    ...
    const rows = await runSql(sqlToRun);
