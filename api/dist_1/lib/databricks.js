"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.runSql = runSql;
const env_js_1 = require("./env.js");
function sleep(ms) {
    return new Promise((r) => setTimeout(r, ms));
}
async function dbxFetch(path, init) {
    const url = `${env_js_1.ENV.DATABRICKS_HOST}${path}`;
    const res = await fetch(url, {
        ...init,
        headers: {
            "Authorization": `Bearer ${env_js_1.ENV.DATABRICKS_TOKEN}`,
            "Content-Type": "application/json",
            ...(init?.headers ?? {})
        }
    });
    if (!res.ok) {
        const text = await res.text().catch(() => "");
        throw new Error(`Databricks API error ${res.status}: ${text}`);
    }
    return res.json();
}
/**
 * Runs a SQL statement on a Databricks SQL Warehouse and returns rows as array of objects.
 * Keep result sets SMALL (always LIMIT) for hackathon speed.
 */
async function runSql(statement, timeoutMs = 15000) {
    const body = {
        warehouse_id: env_js_1.ENV.DATABRICKS_WAREHOUSE_ID,
        catalog: env_js_1.ENV.DATABRICKS_CATALOG,
        schema: env_js_1.ENV.DATABRICKS_SCHEMA,
        statement,
        disposition: "INLINE",
        format: "JSON_ARRAY"
    };
    const created = await dbxFetch(`/api/2.0/sql/statements/`, {
        method: "POST",
        body: JSON.stringify(body)
    });
    const start = Date.now();
    while (Date.now() - start < timeoutMs) {
        const polled = await dbxFetch(`/api/2.0/sql/statements/${created.statement_id}`, {
            method: "GET"
        });
        const state = polled.status.state;
        if (state === "SUCCEEDED") {
            const cols = polled.result?.schema?.columns?.map(c => c.name) ?? [];
            const rows = polled.result?.data_array ?? [];
            return rows.map(r => Object.fromEntries(r.map((v, i) => [cols[i] ?? `c${i}`, v])));
        }
        if (state === "FAILED" || state === "CANCELED") {
            throw new Error(`SQL ${state}: ${polled.status.error?.message ?? "Unknown error"}`);
        }
        await sleep(250);
    }
    throw new Error(`SQL timeout after ${timeoutMs}ms`);
}
